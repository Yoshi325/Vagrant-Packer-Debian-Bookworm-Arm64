...
echo "[INFO ] InstallParallelsGuestTools Progressing."
...
echo "[INFO ] InstallParallelsGuestTools is Complete."
...
rm -rf /var/lib/dhcp/*
# rm -rf /var/log/* # leave logs; one reason is the final system info below
rm -rf /var/tmp/{..?*,.[!.]*,*}
...
printf 'Parallels Guest Tools: '; grep 'installation of Parallels Guest Tools version' /var/log/parallels-tools-install.log | tail -n 1 | sed "s/^Started installation of Parallels Guest Tools version '\([0-9\.]*\)'$/\1/"
echo '--------------------------------------------------'
